...
function SectionCard({ children, id }: { children: React.ReactNode; id?: string }) {
  return (
    <section id={id} style={{ background: "#0F172A", borderRadius: 24, border: "1px solid rgba(255,255,255,0.05)", boxShadow: "0 8px 32px rgba(0,0,0,0.3)", overflow: "hidden" }}>
      {children}
    </section>
  );
}
...
function SectionHeader({ icon: Icon, iconColor, bgGradient, title, subtitle }: {
  ...
  return (
    <div style={{ background: bgGradient, padding: 24, borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div>
          <h3 style={{ fontSize: 18, fontWeight: 700, color: "#F1F5F9", margin: 0, fontFamily: "var(--font-display)" }}>{title}</h3>
          <span style={{ fontSize: 11, fontWeight: 600, color: "#94A3B8", textTransform: "uppercase", letterSpacing: "0.08em", fontFamily: "var(--font-display)" }}>{subtitle}</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      <div style={{ width: "100%", maxWidth: 1152, margin: "0 auto", padding: "24px 16px 96px", display: "flex", flexDirection: "column", gap: 32 }}>
        ...
        {/* Page Title */}
        <div>
          <h2 style={{ fontSize: 22, fontWeight: 700, letterSpacing: "-0.025em", color: "#F1F5F9", margin: 0, fontFamily: "var(--font-display)" }}>Tax Planning</h2>
          <p style={{ fontSize: 13, color: "#64748B", marginTop: 2, fontFamily: "var(--font-display)" }}>FY 2026-27 · Personalised tax analysis</p>
        </div>
        ...
        <div id="regime" style={{ borderRadius: 16, padding: "20px 24px", border: "1px solid rgba(16,185,129,0.3)", background: "rgba(16,185,129,0.1)" }}>
          <h3 style={{ fontWeight: 700, fontSize: 18, display: "flex", alignItems: "center", gap: 8, color: "#10B981", margin: 0, marginBottom: 4, fontFamily: "var(--font-display)" }}>
            <Sparkles size={20} />
            RECOMMENDATION: Choose <span style={{ color: "#F1F5F9", marginLeft: 4 }}>{recommended === "old" ? "OLD" : "NEW"} REGIME</span>
          </h3>
          <p style={{ color: "#F1F5F9", fontSize: 20, fontWeight: 700, marginBottom: 8, fontFamily: "var(--font-display)" }}>You SAVE {savingsFormatted}</p>
          <p style={{ color: "#94A3B8", fontSize: 13, lineHeight: 1.5, margin: 0, fontFamily: "var(--font-display)" }}>
            {recommended === "old"
        ...
        {/* Comparison Table */}
        <div style={{ background: "#0F172A", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 16, overflow: "hidden", boxShadow: "0 4px 24px rgba(0,0,0,0.3)" }}>
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.1)" }}>
                  <th style={{ textAlign: "left", padding: "16px 20px", fontSize: 13, fontWeight: 700, color: "#94A3B8", width: "40%" }}></th>
                  <th style={{
                    textAlign: "center", padding: "16px 20px", fontSize: 13, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", width: "30%",
                    color: recommended === "old" ? "#10B981" : "#94A3B8",
                  ...
                  <th style={{
                    textAlign: "center", padding: "16px 20px", fontSize: 13, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", width: "30%",
                    color: recommended === "new" ? "#10B981" : "#94A3B8",
              ...
              <tbody>
                {[
                  ...
                  >
                    <td style={{ padding: "12px 20px", fontSize: row.final ? 15 : 13, color: row.final ? "#F1F5F9" : row.bold ? "#F1F5F9" : "#CBD5E1", fontWeight: row.final ? 700 : row.bold ? 700 : 400, fontFamily: "var(--font-display)" }}>
                      {row.label}
                    </td>
                    <td style={{
                      ...
                      color: row.final ? (recommended === "old" ? "#F1F5F9" : "#CBD5E1") : "#CBD5E1",
                      background: recommended === "old" ? "rgba(16,185,129,0.05)" : "transparent",
                    }}>
                      {fmt(row.old)}
                    </td>
                    <td style={{
                      ...
                      fontSize: row.final ? 17 : 13,
                      fontWeight: row.final || row.bold ? 700 : 400,
                      color: row.final ? (recommended === "new" ? "#10B981" : "#CBD5E1") : "#CBD5E1",
                      background: recommended === "new" ? "rgba(16,185,129,0.05)" : "transparent",
                    }}>
                      {fmt(row.new)}
                    </td>
        ...
        <SectionCard id="tds">
          ...
          <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              <div style={{ background: "#0B0F1A", borderRadius: 12, border: "1px solid rgba(255,255,255,0.05)", padding: 16, textAlign: "center" }}>
                <p style={{ fontSize: 11, fontWeight: 600, color: "#94A3B8", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 4, fontFamily: "var(--font-display)" }}>Estimated Tax Liability</p>
                <p style={{ fontSize: 18, fontWeight: 700, color: "#F1F5F9", margin: 0, fontFamily: "var(--font-display)" }}>{tds.recommendedTaxFormatted}</p>
                <p style={{ fontSize: 11, color: "#64748B", marginTop: 2, fontFamily: "var(--font-display)" }}>{recommended === "old" ? "Old Regime" : "New Regime"}</p>
              </div>
              <div style={{ background: "#0B0F1A", borderRadius: 12, border: "1px solid rgba(255,255,255,0.05)", padding: 16, textAlign: "center" }}>
                <p style={{ fontSize: 11, fontWeight: 600, color: "#94A3B8", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 4, fontFamily: "var(--font-display)" }}>TDS Deducted (Est.)</p>
                <p style={{ fontSize: 18, fontWeight: 700, color: "#F1F5F9", margin: 0, fontFamily: "var(--font-display)" }}>{tds.totalTDSFormatted}</p>
                <p style={{ fontSize: 11, color: "#64748B", marginTop: 2, fontFamily: "var(--font-display)" }}>@10% of salary</p>
              </div>
            </div>
            {tds.status === "refund" && (
              <div style={{ display: "flex", alignItems: "flex-start", gap: 12, background: "rgba(52,211,153,0.1)", border: "1px solid rgba(52,211,153,0.2)", borderRadius: 12, padding: 16 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 13, color: "#94A3B8", lineHeight: 1.5, margin: 0, fontFamily: "var(--font-display)" }}>File your ITR before 31 July to claim it.</p>
                </div>
              </div>
            )}
            {tds.status === "due" && (
              <div style={{ display: "flex", alignItems: "flex-start", gap: 12, background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 12, padding: 16 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 13, color: "#94A3B8", lineHeight: 1.5, margin: 0, fontFamily: "var(--font-display)" }}>Pay as Advance Tax before 15 March to avoid interest.</p>
                </div>
              </div>
            )}
            {tds.status === "matched" && (
              <div style={{ display: "flex", alignItems: "flex-start", gap: 12, background: "rgba(14,165,233,0.1)", border: "1px solid rgba(14,165,233,0.2)", borderRadius: 12, padding: 16 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 13, color: "#94A3B8", lineHeight: 1.5, margin: 0, fontFamily: "var(--font-display)" }}>No additional payment required.</p>
                </div>
              </div>
            )}
          </div>
        ...
        {rental.hasRentalIncome && (
          <SectionCard>
            ...
            <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 16 }}>
              <div style={{ background: "#0B0F1A", borderRadius: 12, border: "1px solid rgba(255,255,255,0.05)", padding: 16, display: "flex", flexDirection: "column", gap: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}>
                  <span style={{ color: "#CBD5E1", fontFamily: "var(--font-display)" }}>Gross Rental Income</span>
                  <span style={{ fontWeight: 700, color: "#F1F5F9", fontFamily: "monospace" }}>{rental.grossFormatted}</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}>
                ...
                <div style={{ borderTop: "1px dashed rgba(255,255,255,0.1)", paddingTop: 10, display: "flex", justifyContent: "space-between", fontSize: 13 }}>
                  <span style={{ fontWeight: 700, color: "#F1F5F9", fontFamily: "var(--font-display)" }}>Net Rental Added to Income</span>
          ...
          <div style={{ padding: 24 }}>
            {deductions.isOldRegime ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <div style={{ background: "#0B0F1A", borderRadius: 12, border: "1px solid rgba(255,255,255,0.05)", overflow: "hidden" }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr auto auto", gap: 24, padding: "10px 16px", borderBottom: "1px solid rgba(255,255,255,0.05)", fontSize: 11, fontWeight: 700, color: "#94A3B8", textTransform: "uppercase", letterSpacing: "0.08em" }}>
                  ...
                  {deductions.items.map((item, i) => (
                    <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr auto auto", gap: 24, alignItems: "center", padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                      <div>
                        <p style={{ fontSize: 13, fontWeight: 700, color: "#F1F5F9", margin: 0, fontFamily: "var(--font-display)" }}>{item.label}</p>
                        <p style={{ fontSize: 11, color: "#64748B", margin: 0, fontFamily: "var(--font-display)" }}>{item.sublabel}</p>
                      </div>
                      <span style={{ fontSize: 13, fontWeight: 700, color: "#F1F5F9", textAlign: "right", whiteSpace: "nowrap", fontFamily: "monospace" }}>
              ...
              <div style={{ background: "#0B0F1A", borderRadius: 12, border: "1px solid rgba(255,255,255,0.05)", padding: 20 }}>
                <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                  <Info size={20} style={{ color: "#818CF8", flexShrink: 0, marginTop: 2 }} />
                  <div>
                    <p style={{ fontSize: 13, color: "#94A3B8", lineHeight: 1.5, margin: 0, marginBottom: 8, fontFamily: "var(--font-display)" }}>
                      Deductions under 80C, 80CCD(1B), and HRA are <span style={{ fontWeight: 700, color: "#F1F5F9" }}>not available</span> under the New Tax Regime. Only the ₹75,000 standard deduction applies.
                    </p>
                    <p style={{ fontSize: 12, color: "#64748B", margin: 0, fontFamily: "var(--font-display)" }}>
