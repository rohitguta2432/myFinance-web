    ...
    return (
        <section id="prob" style={{ padding: "clamp(80px, 10vw, 120px) 0" }}>
            <div style={{ maxWidth: 1120, margin: "0 auto", padding: "0 2rem" }}>
                ...
                <p style={{ fontFamily: "var(--font-serif)", fontSize: 16, color: palette.mute, lineHeight: 1.75, maxWidth: 520, fontStyle: "italic" }}>
                ...
                <div className="bento-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginTop: 40 }}>
                    {problems.map((p) => (
                        <div key={p.title} className="bento-card">
                            <div style={{ width: 36, height: 36, borderRadius: 9, background: palette.s2, border: `1px solid ${palette.brd}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, marginBottom: 14 }}>
                                {p.icon}
                            </div>
                            <div style={{ fontFamily: "var(--font-display)", fontSize: 14, fontWeight: 700, marginBottom: 6, letterSpacing: "-0.01em", color: palette.txt }}>{p.title}</div>
                            <div style={{ fontSize: 13, color: palette.mute, lineHeight: 1.6 }}>{p.desc}</div>
